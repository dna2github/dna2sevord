   ...
   var SkeletonBinary = (function () {
      ...
      SkeletonBinary.prototype.readSkeletonData = function (binary) {
         ...
         for (var i = 0; i < n; i++) {
            ...
            data.transformMode = 2 + SkeletonBinary.TransformModeValues[input.readInt(true)];
         ...
         for (var i = 0, nn = void 0; i < n; i++) {
            ...
            data.skinRequired = input.readBoolean();
            data.bones = skeletonData.bones.slice();
            nn = input.readInt(true);
            for (var ii = 0; ii < nn; ii++)
               data.bones.push(skeletonData.bones[input.readInt(true)]);
            data.target = skeletonData.bones[input.readInt(true)];
         ...
         if (defaultSkin != null) {
            ...
            defaultSkin.bones = skeletonData.bones.slice();
      ...
      SkeletonBinary.prototype.readSkin = function (input, skeletonData, defaultSkin, nonessential) {
         ...
         if (defaultSkin) {
            ...
            skin.bones = skeletonData.bones.slice();
            slotCount = input.readInt(true);
         }
      ...
      SkeletonBinary.prototype.readAnimation = function (input, name, skeletonData) {
         ...
         for (var i = 0, n = input.readInt(true); i < n; i++) {
            ...
            for (var frameIndex = 0; frameIndex < frameCount; frameIndex++) {
               // frameIndex, time, mix, softness, bendDirection, compress, strentch
               timeline.setFrame(
                  frameIndex, input.readFloat(), input.readFloat(),
                  0, input.readByte(), false, false
               );
               if (frameIndex < frameCount - 1)
                  this.readCurve(input, frameIndex, timeline);
            }
